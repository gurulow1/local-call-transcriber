...
Для AAC ADTS дополнительно нужен зафиксированный PyAV wheel из `requirements/aac.txt`; процедура загрузки во время staging и офлайн-установки приведена в разделе 1. Whisper декодирует AAC во временный PCM WAV 16 кГц внутри каталога результата, сохраняет stereo для разметки каналов и удаляет временный файл после обработки.
