...
## Запуск из распакованного архива Windows

На 64-битном Windows-компьютере с NVIDIA дважды нажать [`START_TRANSCRIBER.cmd`](START_TRANSCRIBER.cmd). Первый запуск:

1. проверяет свободное место и NVIDIA driver;
2. находит 64-битный Python 3.11 либо скачивает официальный Python 3.11.9 с проверкой размера и SHA-256;
3. создаёт `.venv` и устанавливает только hash-pinned Windows wheels;
4. скачивает и проверяет официальный `whisper.cpp v1.9.2` и `Whisper large-v3` (около 3,8 ГБ);
5. запускает полный набор unit/contract tests;
6. запрашивает административное подтверждение для outbound firewall;
7. открывает `data/input` и запускает постоянный локальный worker.

После появления сообщения `Setup complete` положить в открывшуюся папку искусственный или разрешённый тестовый WAV/MP3/FLAC/OGG/AAC. Результат появится в `data/calls/{call_id}/`. Повторный запуск использует уже подготовленное окружение и локальные артефакты. До согласования доступа, хранения и проверки ИБ реальные корпоративные звонки не использовать.
